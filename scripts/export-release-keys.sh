...
#   SIGNING_APP_NAME   - used for output file naming (ex: tuvan)
...
if [ -n "${MINISIGN_KEY}" ] || [ -n "${MINISIGN_PUB}" ]; then
    ...
    echo "Skipping minisign public key export (set MINISIGN_KEY or MINISIGN_PUB to enable)"
fi
...
if [ -n "${PGP_KEY_ID}" ]; then
    ...
    echo "Skipping PGP public key export (set PGP_KEY_ID to enable)"
fi
